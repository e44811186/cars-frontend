document.addEventListener("DOMContentLoaded", () => {
  ...
  function createCarArticle(car) {
    ...
    article.innerHTML = `
      <img src="${car.imageUrl}" alt="car" loading="lazy">
      <div class="car-details">
        <h4>${car.brand} ${car.model} (${car.year})</h4>
        <p>${car.description}</p>
        <div class="car-action">
          <ul>
            ${["на 1 сутки","на 1-3 суток","на 3+ суток"].map(
              (p, i) => `
              <li>
                <div class="car-period">${p}</div>
                <div class="car-price">${prices[i]} $ ${
                i > 0 ? "<span>/сут</span>" : ""
              }</div>
              </li>`
            ).join("")}
          </ul>
          <a href="#order" class="button white-button" data-title="${
            car.brand
          } ${car.model}">Забронировать</a>
        </div>
      </div>
    `;
